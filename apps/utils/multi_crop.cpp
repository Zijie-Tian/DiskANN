...
template<typename T, typename TagT = uint32_t>
void multi_shard(const std::string &data_file, const std::string &dst_prefix,
                 std::vector<size_t> &npts_arr, size_t &aligned_dim) {
  ...
  for (auto npts : npts_arr) {
    std::vector<uint32_t> base_tags(tags.begin(), tags.begin() + npts);
    dump_to_disk<T, TagT>(all_points, aligned_dim,
                          dst_prefix + "_" + std::to_string(npts), base_tags);
  }
}
...
int main(int argc, char const *argv[]) {
  if (argc < 5) {
    std::cout << "Correct usage: " << argv[0]
              << " <type[int8/uint8/float]> <datafile> <dst_prefix> <npts ...>"
              << "\n WARNING: Other parameters set inside CPP source."
              << std::endl;
    exit(-1);
  }

  std::vector<size_t> npts_arr;
  for (int i = 4; i < argc; i++) {
    npts_arr.emplace_back(atoi(argv[i]));
  }

  size_t aligned_dim;
  if (strcmp(argv[1], "int8") == 0) {
    multi_shard<int8_t, uint32_t>(argv[2], argv[3], npts_arr, aligned_dim);
  } else if (strcmp(argv[1], "uint8") == 0) {
    multi_shard<uint8_t, uint32_t>(argv[2], argv[3], npts_arr, aligned_dim);
  } else if (strcmp(argv[1], "float") == 0) {
    multi_shard<float, uint32_t>(argv[2], argv[3], npts_arr, aligned_dim);
  } else {
